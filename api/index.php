...
// Configuration
$ROOT_DIR = __DIR__ . DIRECTORY_SEPARATOR . 'eoffice_files';
...
function get_safe_path($filename, $base_dir) {
    if (empty($filename)) return null;
    $target = realpath($base_dir . DIRECTORY_SEPARATOR . $filename);
    $base = realpath($base_dir);
    if (!$target) {
        $target = htmlspecialchars($base_dir . DIRECTORY_SEPARATOR . basename($filename));
        return $target;
    }
    return (strpos($target, $base) === 0) ? $target : null;
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'create':
                ...
                if (!empty($filename)) {
                    $path = get_safe_path($filename, $ROOT_DIR);
                    if ($path && !file_exists($path)) {
                        file_put_contents($path, "");
                        $success_msg = "Blank file initialized successfully.";
                    } else { $error_msg = "File already exists or name is invalid."; }
                }
                break;
            ...
            case 'upload':
                if (isset($_FILES['uploaded_file']) && $_FILES['uploaded_file']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $path = get_safe_path($original_name, $ROOT_DIR);
                    
                    if ($path) {
                        if (move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $path)) {
                            $success_msg = "File '$original_name' uploaded successfully.";
                        } else { $error_msg = "Failed to move uploaded file."; }
                    } else { $error_msg = "Invalid filename layout."; }
            ...
            case 'save_notepad':
                ...
                if ($path && file_exists($path) && is_file($path)) {
                    ...
                    $success_msg = "Changes written directly to disk.";
                }
                break;
            ...
            case 'rename':
                ...
                $old_path = get_safe_path($old_name, $ROOT_DIR);
                $new_path = get_safe_path($new_name, $ROOT_DIR);
                if ($old_path && file_exists($old_path) && $new_path && !file_exists($new_path) && !empty($new_name)) {
            ...
            case 'delete':
                ...
                if ($path && file_exists($path) && is_file($path)) {
                    unlink($path);
                    $success_msg = "File erased permanently.";
                }
                break;
...
if (!empty($active_file)) {
    $target_path = get_safe_path($active_file, $ROOT_DIR);
    if ($target_path && file_exists($target_path) && is_file($target_path)) {
        if ($is_editable_text) {
            $edit_file_content = file_get_contents($target_path);
        }
        $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https://" : "http://";
        $current_folder_url = $protocol . $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI']);
        $file_web_url = rtrim($current_folder_url, '/') . '/eoffice_files/' . rawurlencode($active_file);
    } else {
        $active_file = "";
    }
}
...
<head>
    ...
    <style>
        ...
        * { scrollbar-width: thin; scrollbar-color: var(--scrollbar-thumb) var(--bg-main); box-sizing: border-box; }
        ...
        .file-list { flex: 1; list-style: none; }
        ...
        /* Media Frame Responsive Adapters */
    ...
    <div class="app-layout">
        ...
        <div class="main-workspace">
            <aside class="sidebar">
                ...
                <ul class="file-list">
                    <?php
                    $entries = array_diff(scandir($ROOT_DIR), array('.', '..'));
                    sort($entries);
                    if (empty($entries)):
                        ...
                        foreach ($entries as $index => $name):
                            if(is_dir($ROOT_DIR . DIRECTORY_SEPARATOR . $name)) continue;
                            ?>
                            <li class="file-item">
